...
import { extractSeatNumber, formatDateToDayMonth, formatTime, getCityShortForm, getDayShortName } from './HelperFunctions';

const TicketCard = ({ filterType }) => {
  const tickets = useSelector(state => state.tickets.data);
  const filteredTickets = filterType === 'active' ? tickets?.active || [] : filterType === 'completed' ? tickets?.past || [] : [];
  ...
  return (
    <div className="flex flex-col md:grid grid-cols-3 items-center justify-center bg-center bg-cover">
      {filteredTickets.map((ticket, index) => (
        <div key={index} className="max-w-md w-full h-full mx-auto z-10 rounded-3xl">
                ...
                <div className="flex-auto justify-evenly">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center my-1">
                      <h2 className="font-medium italic ml-2">{ticket?.adminName}</h2>
                    </div>
                    <div className="ml-auto text-secondary">{ticket?.busDetails?.busNumber}</div>
                  </div>
                  <div className="border-dashed border-b-2 my-5"></div>
                  <div className="flex items-center">
                    <div className="flex flex-col">
                      <div className="flex-auto text-xs text-ternary my-1">
                        <span className="mr-1">{getDayShortName(ticket?.date)}</span>
                    ...
                    <div className="flex flex-col mx-auto">
                      <img src={ticket?.busDetails?.imageSrc} className="h-14 p-1" alt="Bus Logo" />
                    </div>
                    <div className="flex flex-col">
                      <div className="flex-auto text-xs text-ternary my-1">
                        <span className="mr-1">{ticket?.date ? getDayShortName(new Date(ticket?.date)) : 'N/A'}</span>
                        <span>{formatDateToDayMonth(ticket?.date)}</span>
                      </div>
                      <div className="w-full flex-none text-lg text-secondary font-bold leading-none">
                  ...
                  <div className="flex items-center mb-5 p-5 text-sm">
                    <div className="flex flex-col">
                      ...
                      <div className="font-semibold">{ticket.busDetails?.busNumber}</div>
                    </div>
                  ...
                  <div className="flex justify-between items-center mb-4 px-5">
                    <div className="flex flex-col text-sm">
                      <span>Departure</span>
                      <div className="font-semibold">{formatTime(ticket?.departureTime)}</div>
                    </div>
                    <div className="flex flex-col text-sm">
                      <span>Arrival</span>
                      <div className="font-semibold">{formatTime(ticket?.arrivalTime)}</div>
                    </div>
                  </div>
                  <div className="border-dashed border-b-2 my-5 pt-5 relative">
                    <div className="absolute rounded-full w-5 h-5 bg-primary -mt-2 -left-2"></div>
                  ...
                  <div className="flex items-center px-5 pt-3 text-sm">
                    ...
                    <div className="flex flex-col mx-auto">
                      <span>Class</span>
                      <div className="font-semibold">{ticket?.busDetails?.standard.toUpperCase()}</div>
                    </div>
                    <div className="flex flex-col">
                      <span>Seat</span>
                      <div className="font-semibold">{extractSeatNumber(ticket?.seatNumber)}</div>
                    </div>
                  </div>
                  <div className="flex flex-col py-5 justify-center text-sm">
                    <h6 className="text-ternary text-center text-sm">Show this to driver in the bus</h6>
