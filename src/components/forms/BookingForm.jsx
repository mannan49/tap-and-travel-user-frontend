...
import { isSameUTCDate } from '../utils/isSameUtcDate';
...
const BookingForm = () => {
  ...
  useEffect(() => {
    const fetchBuses = async () => {
      try {
        ...
        const uniqueCities = Array.from(new Set(data.flatMap(bus => [bus.route.startCity, bus.route.endCity])));
  ...
  const filterBuses = () => {
    ...
    const results = buses.filter(bus => {
      ...
      if (isNaN(busDate) || isNaN(selectedDate)) {
        console.error('Invalid dates:', bus.date, date);
        return false;
      }
      ...
      const datesMatch = isSameUTCDate(busDate, selectedDate);

      if (citiesMatch && !datesMatch) {
        console.log('❌ Date mismatch despite visual match:', {
          selected: selectedDate.toISOString(),
          bus: busDate.toISOString(),
        });
      }
  ...
  return (
    <div>
      <h1 className="text-center font-bold text-2xl mb-2">Let&apos;s Book a Ride</h1>
      <div className="flex flex-col lg:grid grid-cols-10">
        <div className="col-span-4 flex justify-center items-center">
          <img src={`https://www.freeiconspng.com/uploads/bus-png-4.png`} alt={`Bus Picture 1`} className="w-full" />
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
          {filteredBuses.map(bus => (
            <div key={bus._id} className="bg-white rounded-lg shadow-lg p-6 transition transform hover:scale-105 hover:shadow-xl">
              <p className="text-gray-600 mb-2 app-btn text-center">{bus?.route?.startCity + ' to ' + bus?.route?.endCity}</p>
              <p className="text-gray-600 mb-2">Date: {new Date(bus?.date).toDateString()}</p>
              <p className="text-gray-600 mb-2">Time: {formatTime(bus?.departureTime) + ' to ' + formatTime(bus?.arrivalTime)}</p>
        ...
        hasSearched && <p className="text-center text-gray-600 mt-4">No buses available for this route on the selected date.</p>
      )}
    </div>
  );
};
